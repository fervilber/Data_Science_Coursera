...
# Lectura del fichero con los datos EPA  para los ejemplos
    ...
    head(pollution, 3)
```
Para estos apuntes hemos usado además, los datos de ejemplo incluidos en la librería `datasets`, en concreto la tabla 'cars`.


## Gráficos simples de 1 variable <a name="g1d"></a>

La forma más sencilla de ver los datos de una tabla es haciendo una gráfica rápida y sencilla de las variables importantes de la misma. Empezaremos con los gráficos más sencillos de una variable, para ello la forma más rápida es usar alguna de las siguientes funciones de R:
...
Veamos unos ejemplos de código, en los que dibujamos un boxplot o grafica de cajas que representa de forma sencilla una variable y un histograma de frecuencias de la misma
```{r eval=TRUE}
...
Para pintar dos histogramas juntos podemos usar el atributo `add=TRUE`.
Veamos un ejemplo en el que generamos 2 series aleatorias de 500 observaciones y medias 4 y 6. Usamos en los colores elcomando `alpha=0.5` para generar transparencia.
...
Para echar un ojo a varias variables al mismo tiempo y compararlas usaremos funciones como `boxplot` o `with`.
...
Existe la opción de dividir el lienzo en varios apartados con las múltiples opciones de la función `par()`.
que detallaremos más adelante, pero resaltamos `mfrow=c(num_filas,num,col)` pues es muy usada para crear multigraficas juntas. Duvide el lienzo en subgraficas según el numeor de filas y col marcados.
...
Se trata de funciones poderosas, pues automáticamente nos sirve tambien para distinguir valores según un factor.
Es decir que podemos pintar de distintos colores y agrupar datos según una de las variables, si es o transformamos esta variable en un fator. 

```{r eval=TRUE}
# creamos una multigrafica de 1x2 
    par(mfrow=c(1,2))
...
Para ver qué dispositivos gráficos hay en nuestro sistema escribimos `?Devices`.

## Cómo se crea un gráfico

Llamando explícitamente al dispositivo se crea.
Una vez aierto se convierte en el dispositivo activo y sobreescribe en él.
**__Para cerrar especificamente un dispositivo__** llamamos a `dev.off()`
```{r eval=FALSE}
# Llamamos un pdf
pdf(file = "myplot.pdf")
