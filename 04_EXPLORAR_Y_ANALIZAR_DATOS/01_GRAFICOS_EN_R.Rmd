...
exploratory graphs are the initial step in an investigation, the "quick and dirty" tool used to point the
| data scientist in a fruitful direction. A scientist might need to make a lot of exploratory graphs in order to
| develop a personal understanding of the problem being studied. Plot details such as axes, legends, color and size
| are cleaned up later to convey more information in an aesthetically pleasing way
...
#### Introducción
Al igual que el sistema base, Lattice tiene unas funciones para el trazado. Es un sistema mas conveniente para graficos condicionales, y ajusta por defecto muchas cosas. La desventaja es que no se puede sobrescribir una vez pintado.
Tampoco es muy intuitivo en cambios de detalles
...
ggplot2 es una mezcla de lattice y base system. Lo más importante respecto a **lattice** es que permite sobrescribir encima de una gráfico y que usa una gramática grafica que lo hace más intuitivo.

#### ¿Qué es ggplot2?

* Un desarrollo de gramática grafica(gg).(autor Hadley Wickham)
* Es un sistema de graficos en 3D para R.
* gramatica gráfica es una representacion abstracta de graficos, ideas y objetos.
...
* Los graficos generados son automaticamente ajustados para aparentar estetica y geometría bella.
* Para hacer subconjuntos se usan Factores etiquetados.
...
str(mpg)
head(mpg)
# para ver cuantos datos hay por modelo de coche usamos comando table
table(mpg$model)
